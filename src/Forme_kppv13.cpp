...
#define GOOD(X) ((X>=0)&&(X<13))?X:400
#define GOOD39(X) ((X>=0)&&(X<27))?X:400
#define K_MAX 255
...
int Forme_kppv13::cercle_no_vers_x [NB_CERCLES][24];		// Les no commencent à 0
int Forme_kppv13::cercle_no_vers_y [NB_CERCLES][24];
int Forme_kppv13::x_y_vers_anneau[37][37];		// arguments : 18+i
int Forme_kppv13::x_y_vers_no[37][37];
int Forme_kppv13::contenances_triees[NB_CERCLES+1];
int Forme_kppv13::contenances_c[NB_CERCLES+1];              	// Contenances cumulées
double Forme_kppv13::urgences_noir[GOBAN_SIZE13_2+1];
double Forme_kppv13::urgences_blanc[GOBAN_SIZE13_2+1];
int Forme_kppv13::table_pos_idx_vers_idx[TAILLE_TABLE_POS_IDX_VERS_IDX];
Liste * Forme_kppv13::liste_des_patterns_blanc;
Liste * Forme_kppv13::liste_des_patterns_noir;
Arbre13 * Forme_kppv13::racine_arbre_blanc;
Arbre13 * Forme_kppv13::racine_arbre_noir;
Arbre13 * Forme_kppv13::racine_arbre_nv;
Arbre13 * Forme_kppv13::racine_arbre_nv_bord;
Arbre13 * Forme_kppv13::racine_arbre;
Arbre13 * Forme_kppv13::arbre_retour;
ifstream * Forme_kppv13::ifile;
ofstream * Forme_kppv13::ofile;
double Forme_kppv13::nb_coups;

//int Forme_kppv13::tailles_urgences[GOBAN_SIZE13_2+1];
//int Forme_kppv13::quarantaines[362];
int Forme_kppv13::the_goban[GOBAN_SIZE13_2+1];
int Forme_kppv13::goban_recure[GOBAN_SIZE13_2+1];

// Liens
int Forme_kppv13::goban_construction[1521];
int Forme_kppv13::table_pos_idx_vers_idx_construction[6845];

// Probas
	int Forme_kppv13::n_joue;	
	int Forme_kppv13::n_test;
	int Forme_kppv13::nb_de_patterns_total;
	int Forme_kppv13::nb_de_patterns_sauves[30];
	int Forme_kppv13::nb_de_patterns_affiches;
	int Forme_kppv13::nb_de_patterns_lus;
	long long int Forme_kppv13::nb_boucles;

//	int Forme_kppv13::p_joue;

void Forme_kppv13::ecrire_damier(int damier [GOBAN_SIZE13_2+1]){
int i;
int j;
int Cho=0;
char * c;
...
cout << endl;
cout<<"a b c d e f g h j k l m n "<<endl;
cout << endl;
}
...
void Forme_kppv13::creer_les_listes(void){
//racine_arbre_noir=new Arbre13(NULL,NULL,NULL,NULL,NULL);
//racine_arbre_blanc=new Arbre13(NULL,NULL,NULL,NULL,NULL);
racine_arbre_nv=new Arbre13(NULL,NULL,NULL,NULL,NULL);
racine_arbre_nv_bord=new Arbre13(NULL,NULL,NULL,NULL,NULL);
racine_arbre=new Arbre13(NULL,NULL,NULL,NULL,NULL);
}

void Forme_kppv13::delete_all()
{
...
void Forme_kppv13::trouver_cercles(int n){
...
//for (k=0;k<GOBAN_SIZE13_2+1;k++) {goban_recure[k]=0;}
...
Forme_kppv13::Forme_kppv13(unsigned char kk,unsigned short int p1, unsigned char p2, long long int ur){
K=kk;
urgence_noir=ur;
urgence_blanc=ur;
...
void Forme_kppv13::fimprimer(ostream & o, int d, int n){//
...
int Forme_kppv13::calculer_coordonees_construction(int i,int no_courant,int anneau_courant){
int abs_courante;
int ord_courante;
int idx_courant;
...
void Forme_kppv13::calculer_table_pos_idx_vers_pos_construction(){
int i;
int j;
int no_courant=0;
...
void Forme_kppv13::effacer_listes(Arbre13 * mon_arbre){
int i;
if (mon_arbre) {
...
int Forme_kppv13::ecrire_pattern_2d(ostream * out){
...
void Forme_kppv13::ecrire_nos_2d(char * nom){
...
void Forme_kppv13::ecrire_tous_les_patterns_nv(double proba, double bonte,double urgence){
unsigned short int tmp=0;
Forme_kppv13::ofile = new ofstream("pattern_noir_nv.dat", ios::out);
if (!(*ofile)) {
	cerr << "erreur : ouverture fichier impossible..." << endl ;
	return;
	}
...
Forme_kppv13::ofile = new ofstream("pattern_blanc_nv.dat", ios::out);
if (!(*ofile)) {
	cerr << "erreur : ouverture fichier impossible..." << endl ;
	return;
	}
...
Forme_kppv13::ofile = new ofstream("pattern_bord_noir_nv.dat", ios::out);
if (!(*ofile)) {
	cerr << "erreur : ouverture fichier impossible..." << endl ;
	return;
	}
...
Forme_kppv13::ofile = new ofstream("pattern_bord_blanc_nv.dat", ios::out);
if (!(*ofile)) {
	cerr << "erreur : ouverture fichier impossible..." << endl ;
	return;
	}
...
void Forme_kppv13::ecrire_le_pattern(void){
Element_kppv * element_courant = liste_pierres;
int Cho=1;int Chi=1;
...
void Forme_kppv13::ecrire_arbre(Arbre13 * mon_arbre,int mon_no,double proba, double bonte,double urgence, int couleur){ // attention !  plus a jour du tout ! apprentissage a chang�tout
double pn,pb;
double bn,bb;
double un,ub;
...
if (mon_arbre) {
//	cout<<" Hauteur dans l'arbre : "<<mon_no<<" Forme_kppv13 :"<<endl;
	if (mon_arbre->pattern_associe) {
...
Arbre13 * Forme_kppv13::grimper(Arbre13 * liane,int i){
Arbre13 * q;

switch (i){
 case (0):
	if (liane->vide==NULL){
		q=new Arbre13(NULL,NULL,NULL,NULL,NULL);
		liane->vide=q;
	}
	q=liane->vide;
 break;
...
Forme_kppv13 * Forme_kppv13::generer_le_pattern_3x3(long long int no){
...
Element_kppv * ep=NULL;
	...
	h_courant++;
     ...
     Forme_kppv13 * P = new Forme_kppv13(1,h_courant,maCouleur,4);
     K_courant++;
...
void Forme_kppv13::generer_patterns_3x3(Arbre13 * arbre_init,int hauteur,long long int no){ // !!!!!!!!!!!!!!!!!!!!!!!! Attention, il n'y a pas de gestion du pattern vide possible avec cette repr�entation !!!
int hmax=8;
...
/*void Forme_kppv13::generer_3x3(Arbre13 * arbre_init){ // pourquoi j'ai pas fait une proc�ure r�urente ? Parceque (2 variables globales, lourd...)!

Arbre13 * arbre_courant[8];
int  i[8];
     ...
     for (int i6=0;i6<4;i6++){
      ...
      for (int i7=0;i7<4;i7++){
       ...
       for (int i8=0;i8<4;i8++){
        ...
        {
        Forme_kppv13 * P = new Forme_kppv13(1,0,0,mon_urgence); 
...
int Forme_kppv13::lire_tous_les_patterns_nv(int longueur){
int i=1;
int s=1;
nb_de_patterns_lus=0;

Forme_kppv13::ifile = new ifstream("pattern_noir_nv.dat", ios::in);
if (!(*ifile)) {return 0;}
while (s==1){
...
Forme_kppv13::ifile = new ifstream("pattern_blanc_nv.dat", ios::in);
if (!(*ifile)) {return 0;}
s=1;
while (s==1){
...
int Forme_kppv13::lire_pattern_nv(int couleur,int & longueur){
char c;
int Lee=1;
int Chang=0;
int pomme;
int indic=0;
Forme_kppv13 * P = new Forme_kppv13(1,1,0,0);
...
if (P==NULL) return 0;
if (Lee<(longueur+1)){P->mettre_pattern_dans_arbre(Forme_kppv13::racine_arbre_nv);nb_de_patterns_lus++;return 1;}else {delete P;return 1;}
// un jour il faudra qu'on détruise P s'il est trop gros ;)
}

int Forme_kppv13::lire_tous_les_patterns_nv_bord(int longueur){
int i=1;
int s=1;
nb_de_patterns_lus=0;

Forme_kppv13::ifile = new ifstream("pattern_bord_noir_nv.dat", ios::in);
if (!(*ifile)) {return 0;}
while (s==1){
...
Forme_kppv13::ifile = new ifstream("pattern_bord_blanc_nv.dat", ios::in);
if (!(*ifile)) {return 0;}
s=1;
while (s==1){
...
int Forme_kppv13::lire_pattern_nv_bord(int couleur,int & longueur){
char c;
int urg;
int Lee=1;
int Chang=0;
...
P->n_joue_sachant_match=0;
if (P==NULL) return 0;
if (Lee<(longueur+1)){P->mettre_pattern_dans_arbre(Forme_kppv13::racine_arbre_nv_bord);nb_de_patterns_lus++;return 1;}else {delete P;return 1;}
}

int Forme_kppv13::lire_tous_les_patterns_noir(void){
int i=1;
int r=0;
int nb_pats_lus=0;
Forme_kppv13 * P=NULL;
Forme_kppv13::ifile = new ifstream("pattern_noir.dat", ios::in);
if (!(*ifile)) {return 0;}
while ((r==0)||(P)){
	r=1;
	P=lire_pattern(1);
...
int Forme_kppv13::lire_tous_les_patterns_blanc(void){
int i=1;
int r=0;
int nb_pats_lus=0;
Forme_kppv13 * P=NULL;
Forme_kppv13::ifile = new ifstream("pattern_blanc.dat", ios::in);
if (!(*ifile)) {return 0;}
while ((r==0)||(P)){
	r=1;
	P=lire_pattern(2);
...
Forme_kppv13 * Forme_kppv13::lire_pattern(int couleur){
char c;
int Lee=1;
int Chang=0;
int pomme;
...
P->n_match_noir=0;
P->n_joue_sachant_match=0;
P->mettre_pattern_dans_arbre(Forme_kppv13::racine_arbre);
...
void Forme_kppv13::match_proba_noir(int prochain_coup){
int i=0;
int j=0;
int k=0;

int temp;
Arbre13 * arbre_courant;

//cout << endl << endl << "Forme_kppv13s qui matchent"<< endl <<endl;

for (i=0;i<GOBAN_SIZE13_2;i++) {if (the_goban[i]==0){ // pour chaque intersection vide
	n_test++;
...
void Forme_kppv13::match_nv(double * tab_urgences,T_IDX * goban,double rapport,int couleur)
{
int i;
int j;
double temp;
int goban2[GOBAN_SIZE13_2+1];
//int tmp;
Arbre13 * arbre_courant;
  for (i=0;i<GOBAN_SIZE13_2;i++) tab_urgences[i]=0;

  for (int idx=0; idx<GOBAN_SIZE13_2; idx++)
    goban2[idx] = goban[idx];
  for (int i=0;i<GOBAN_SIZE13;i++)
    if (goban2[i]==0) goban2[i]=2;
  for (int i=0;i<GOBAN_SIZE13;i++)
    if (goban2[i*GOBAN_SIZE13]==0) goban2[i*GOBAN_SIZE13]=2;
  for (int i=1;i<GOBAN_SIZE13;i++)
    if (goban2[i*GOBAN_SIZE13-1]==0) goban2[i*GOBAN_SIZE13-1]=2;
  for (int i=0;i<GOBAN_SIZE13;i++)
    if (goban2[i+(GOBAN_SIZE13-1)*GOBAN_SIZE13]==0) goban2[i+(GOBAN_SIZE13-1)*GOBAN_SIZE13]=2;
  goban2[GOBAN_SIZE13_2]=0;


for (i=0;i<GOBAN_SIZE13_2;i++) {
	if (goban2[i]==0)
	...
	for (j=1;;j++){
		nb_boucles++; 
			switch (goban2[table_pos_idx_vers_idx[j*GOBAN_SIZE13_2+i]]){
				case (1) :arbre_courant=arbre_courant->noir;break;
				case (-1):arbre_courant=arbre_courant->blanc;break;
				case (2) :arbre_courant=arbre_courant->bord;break;
				case (0) :arbre_courant=arbre_courant->vide;break;
...
//				cout <<temp<<endl;
				//if (temp>0) {
				if (temp>0) {tab_urgences[i]=temp*rapport;}
			}
			if (couleur==-1) {
				temp=arbre_courant->pattern_associe->p_joue_sachant_match_blanc;
//				cout <<temp<<endl;
				//if (temp>0) {
				if (temp>0) {tab_urgences[i]=temp*rapport;}
			}
...
void Forme_kppv13::match(double * tab_urgences,T_IDX * goban,double rapport,int couleur)
{
int i;
int j;
double temp;
...
void Forme_kppv13::match_proba_blanc(int prochain_coup){ // !! n_match noir !!
int i=0;
int j=0;
int k=0;
int temp;
...
//	cout <<"i=  "<<i<<endl;
	...
	for (j=1;;j++){
		Forme_kppv13::nb_boucles++; 
		temp=the_goban[table_pos_idx_vers_idx[j*GOBAN_SIZE13_2+i]];
			switch (temp){
				case (1) :arbre_courant=arbre_courant->noir;break;
				case (-1):arbre_courant=arbre_courant->blanc;break;
				case (2) :arbre_courant=arbre_courant->bord;break;
...
void Forme_kppv13::afficher_arbre(Arbre13 * mon_arbre,int mon_no,double proba,double bonte,unsigned char nfs_demande){ // bonf faut tout revoir ici
double p;
double b;
if (mon_arbre) {
//	cout<<" Hauteur dans l'arbre : "<<mon_no<<" Forme_kppv13 :"<<endl;
...
void Forme_kppv13::detruire_arbre(Arbre13 * arbre_mort){
...
void Forme_kppv13::detruire_feuilles(Arbre13 * arbre_pere){
Arbre13 * arbre_fils;
if (arbre_pere) {
	if (arbre_pere->noir) {
...
void Forme_kppv13::mettre_pattern_dans_arbre(Arbre13 * mon_arbre){
int i=1;
unsigned short int pos;
Arbre13 * arbre_courant=mon_arbre;
...
int Forme_kppv13::calculer_coordonees(int i,int no_courant,int anneau_courant){ // apprentissage : pas mal de trucs modifi� ici
int abs_courante;
int ord_courante;
int idx_courant;
int X;
int Y;   
		X= i % GOBAN_SIZE13+cercle_no_vers_x[anneau_courant][no_courant];// apprentissage X=i-(i/GOBAN_SIZE13)*GOBAN_SIZE13+cercle_no_vers_x[anneau_courant][no_courant];
		abs_courante=((X>=0)&&(X<GOBAN_SIZE13))?X:400; // apprentissage
 		Y=(i/GOBAN_SIZE13)+cercle_no_vers_y[anneau_courant][no_courant]; //apprentissage
		ord_courante=((Y>=0)&&(Y<GOBAN_SIZE13))?Y:400;
		idx_courant=GOBAN_SIZE13*ord_courante+abs_courante;
		if (idx_courant<GOBAN_SIZE13_2) {return idx_courant;} else {return GOBAN_SIZE13_2;}
}

void Forme_kppv13::calculer_table_pos_idx_vers_pos(){
//cout << GOBAN_SIZE13_2<<endl;
//cout << GOBAN_SIZE13<<endl;
int i;
int j;
int no_courant=0;
int anneau_courant=1;
for (j=1;j<1369;j++){
		for (i=0;i<GOBAN_SIZE13_2;i++){
			table_pos_idx_vers_idx[j*GOBAN_SIZE13_2+i]=calculer_coordonees(i,no_courant,anneau_courant);
		}
		no_courant++;
		if (contenances_triees[anneau_courant]==no_courant){
			anneau_courant++;
...
void Forme_kppv13::generer_un_pattern(int damier[GOBAN_SIZE13_2],int mon_idx,unsigned char nfs,int c){
...
//	cout << "On va essayer de lire ds la table à la valeur "<<(j*361+mon_idx)<<endl;
		Ho=Forme_kppv13::table_pos_idx_vers_idx[j*361+mon_idx];
...
//	cout << "bon, damier[Ho] est bien défini, pas de pb et = "<<damier[Ho]<<endl;
		switch (damier[Ho]){
				...
				case (2) :(P->liste_pierres)->couleur=2;(P->liste_pierres)->position=j;Lee++;
						if ((Ho%13==0)||(Ho%13==12)) vertical++;
						if ((Ho/13==0)||(Ho/13==12)) horizontal++;
...
Forme_kppv13::nb_de_patterns_total++;
...
int Forme_kppv13::trouver_urgence_max(double * mon_damier_des_urgences){
int i;
int mon_idx=0;
double max=0;
for (i=0;i<GOBAN_SIZE13_2;i++){
...
void Forme_kppv13::init_urgences(Arbre13 * a,double coef_noir,double exp_noir,double coef_blanc,double exp_blanc,double Rinit,double probaMin)// modification : procedure amelioree pour pouvoir calculer le coeficient optimal.
{
//  cout <<"+"<<endl;
//  cout <<"*";
  if (a==NULL) return;
  if (a->pattern_associe){
    ...
    double Ppb=0;
    if (Pn>0){
	Ppn = ((double)GOBAN_SIZE13_2/381)*(Bn*Bn/(Pn*Pn))/3000000;
...
T_URG Forme_kppv13::de_proba_a_urgence_blanc(double p, double coef_blanc, double exp_blanc)
{
...
T_URG Forme_kppv13::de_proba_a_urgence_noir(double p, double coef_noir,double exp_noir)
{
  return ((long long int)(coef_noir*pow(p,exp_noir)));
}

void Forme_kppv13::tests(){
...
void Forme_kppv13::init_class()
{
  ...
  Forme_kppv13::creer_les_listes();
  Forme_kppv13::trouver_cercles(18);// Determiner la correspondance anneau-goban. Il faut laisser 18 quelque soit la taille du goban (en fait 18*18 est la taille maximale du plus gros patern qui peut �re stoqu�en m�oire...)
  Forme_kppv13::calculer_table_pos_idx_vers_pos();
  tmp=Forme_kppv13::lire_tous_les_patterns_noir(); // Charger tous les patterns en mémoire vive
  PRINTLOG( tmp << " patterns NOIRS dans racine_arbre.");
  if(tmp == 0) {
	  assert(0);
  }

  tmp=Forme_kppv13::lire_tous_les_patterns_blanc();   // Charger tous les patterns en mémoire vive
  PRINTLOG(tmp << " patterns BLANCS dans racine_arbre.");
  if(tmp == 0) {
	  assert(0);
  }
  tmp=Forme_kppv13::lire_tous_les_patterns_nv(KPP_NB_VOISINES); // Charger tous les patterns en mémoire vive
  PRINTLOG( tmp << " patterns reduits dans racine_arbre_nv."); 
  if(tmp == 0) {
	  assert(0);
  }
  tmp=Forme_kppv13::lire_tous_les_patterns_nv_bord(KPP_NB_VOISINES); // Charger tous les patterns en mémoire vive
...
//  Apprentissage_kppv::somme_V_noir=0;
  ...
  //Forme_kppv13::generer_patterns_3x3(racine_arbre_nv,0,0);
  //Forme_kppv13::generer_patterns_3x3(racine_arbre_nv_bord,0,0); // !!!!! attention : on ne peut plus appeller init_urgences !!!!!
  ...
  //Forme_kppv13::calculer_table_pos_idx_vers_pos_construction();
  //cout << " La table a ete realisee pour les liens dans l'arbre "<<endl;
  
  //Forme_kppv13::construire_liens(Forme_kppv13::racine_arbre_77,Forme_kppv13::racine_arbre_77,0,0);
  //cout << "Les liens ont été construit pour l'arbre 77"<<endl;

  //Damier::FKPPV_matcher_tout();
  //for (int i=0;i<362;i++) Forme_kppv13::quarantaines[i]=0;
...
//**********************************************************************************************
/*
void Forme_kppv13::match_recurrent_noir(){
int i;
for (i=0;i<GOBAN_SIZE13_2;i++){
	switch (the_goban[i]){
...
void Forme_kppv13::match_77(int idx_centrale){
int i;
int j;
int temp;
//int tmp;
...
void Forme_kppv13::match_recurrent(int idx_centrale){
int i;
int temp;
urgences[idx_centrale]=0;
for (i=1;i<37;i++) {
...
/*
void Forme_kppv13::match_intersection_recurrent(int idx_testee,Arbre13 * arbre_depart,int hauteur_de_depart){
Arbre13 * arbre_courant=arbre_depart;
...
//**********************************************************************************************
/*
void Forme_kppv13::match_reduit_lineaire(int idx_centrale){
int i;
int j;
int temp;
//int tmp;
...
void Forme_kppv13::match_intersection(int & intersection, Arbre13 * arbre_courant,int hauteur_faite){
int j;
...
//**********************************************************************************************
/*
void Forme_kppv13::ecrire_urgences(void){
int i;
int j;
int Cho=0;
for (i=0;i<19;i++){
...
/*
void Forme_kppv13::construire_liens(Arbre13 * arbre_de_depart,Arbre13 * mon_arbre,int mon_no,int couleur){
int k,i;
...
void Forme_kppv13::lier_pattern(Arbre13 * gros_arbre, Arbre13 * petit_arbre,int haut_pa,int no_lien,double urgence_courante){
...
void Forme_kppv13::verification(void){
/*int k,j;
...
Forme_kppv13::trouver_cercles(18);			// Déterminer la correspondance anneau-goban
...
// Forme_kppv13::symetries();//pas disponibles ici
...
tmp=Forme_kppv13::lire_tous_les_patterns_77();		// Charger tous les patterns en mémoire vive
cout << " Les patterns pour le matching réduits ont été lus, il y en avais : "<<tmp<<endl;

Forme_kppv13::construire_liens(Forme_kppv13::racine_arbre_77,Forme_kppv13::racine_arbre_77,0,0);
cout << "Les liens ont été construit pour l'arbre 77"<<endl;
Forme_kppv13::construire_liens(Forme_kppv13::racine_arbre_noir,Forme_kppv13::racine_arbre_noir,0,0);
cout << "Les liens ont été construit pour l'arbre noir"<<endl;

Forme_kppv13::verification();

//for (i=2;i<21;i++){	Forme_kppv13::afficher_arbre(Forme_kppv13::racine_arbre_77,0,0.01,1.4,i);}


//Forme_kppv13::bien_lie(Forme_kppv13::racine_arbre_noir);

Fichier_pattern::calculer_probas_toutes_les_parties();

// Résultats :
cout << endl;
//cout << " Nombre de patterns sauvés-lus : "<< somme-Forme_kppv13::nb_de_patterns_lus<<endl;
cout << " Nombre de patterns créés : " << Forme_kppv13::nb_de_patterns_total<<endl;
cout << " Nombre de patterns lus : "<< Forme_kppv13::nb_de_patterns_lus<<endl;
//cout << " Nombre de patterns affichés : " << Forme_kppv13::nb_de_patterns_affiches<<endl;
cout << endl;
cout << " Nombre de parties testées : " << Fichier_pattern::nombre_de_parties_testees<<endl;

cout << " Nombre de coups joués : " << Forme_kppv13::n_joue<<endl;
...
cout <<endl;
cout << " Nombre de pattern-matchings : " << Forme_kppv13::n_test<<endl;
cout << " Nombre de comparaisons de pierres : " << Forme_kppv13::nb_boucles<<endl;
cout << " Nombre de comparaisons par pattern-matching : " << (double)Forme_kppv13::nb_boucles/(double)Forme_kppv13::n_test<<endl;
cout << " Nombre de pattern-matching pour un goban: " << (double)Forme_kppv13::n_test/(double)Fichier_pattern::nombre_de_coups_dans_parties_testees<<endl;
*/
/*
int x,y;
x=0;
...
Forme_kppv13::bien_lie(Forme_kppv13::racine_arbre_77);
...
multi=1;
p=1;
	for (k=20;k<(GOBAN_SIZE13_2-19);k++){Forme_kppv13::the_goban[k]=0;} // Ici on met des 2 sur les bords du damier
	for (k=0;k<GOBAN_SIZE13;k++){Forme_kppv13::the_goban[k]=2;}
	for (k=0;k<GOBAN_SIZE13;k++){Forme_kppv13::the_goban[k*19]=2;}
	for (k=1;k<GOBAN_SIZE13;k++){Forme_kppv13::the_goban[k*19-1]=2;}
	for (k=0;k<GOBAN_SIZE13;k++){Forme_kppv13::the_goban[k+18*19]=2;}
	Forme_kppv13::the_goban[GOBAN_SIZE13_2]=0;

	Forme_kppv13::match_noir(Forme_kppv13::the_goban);
	p=Forme_kppv13::trouver_urgence_max(Forme_kppv13::urgences);
	Forme_kppv13::ecrire_urgences();
	Forme_kppv13::the_goban[p]=1;
	if (acceleration==1){Forme_kppv13::match_reduit_lineaire(p);}

	while (p!=0){
//		joueur blanc
//		if (p!=0) {
//			for (k=0;k<GOBAN_SIZE13_2;k++){Forme_kppv13::urgences[k]=0;}
//			for (k=0;k<multi;k++){Forme_kppv13::match_blanc(damier);}
//			Forme_kppv13::ecrire_urgences();
//			cout << Forme_kppv13::trouver_urgence_max()<<endl;
//			p=Forme_kppv13::trouver_urgence_max();
//			damier[p]=-1;
//			n++;
//		}
cout << endl << "Le goban est maintenant : "<< endl;
		Forme_kppv13::ecrire_damier(Forme_kppv13::the_goban);
		...
		if (x==19){
			...
			if (x>9) x-=1;
			x-=1;y=19-y;
			Forme_kppv13::the_goban[x+y*19]=0;
			...
			if (x>9) x-=1;
			x-=1;y=19-y;
			Forme_kppv13::the_goban[x+y*19]=-1;
		...
		if (acceleration==1){
			Forme_kppv13::match_recurrent(p);
			p=Forme_kppv13::trouver_urgence_max(Forme_kppv13::urgences);
		} else {
			for (k=0;k<GOBAN_SIZE13_2;k++){Forme_kppv13::urgences[k]=0;}
			Forme_kppv13::match_blanc(Forme_kppv13::the_goban);
			p=Forme_kppv13::trouver_urgence_max(Forme_kppv13::urgences);
			if (p==x+y*19) {cout <<"Le pattern matching aurait fait pareil ;)"<<endl;}
		}
		Forme_kppv13::the_goban[x+y*19]=-1;
		Fichier_pattern::determiner_et_enlever_pierres_mortes(x+y*19,Forme_kppv13::the_goban,-1);

		// joueur noir 
		if (acceleration==1){
			Forme_kppv13::match_recurrent(x+y*19);
			p=Forme_kppv13::trouver_urgence_max(Forme_kppv13::urgences);
			Forme_kppv13::ecrire_urgences();
		} else {
			for (k=0;k<GOBAN_SIZE13_2;k++){Forme_kppv13::urgences[k]=0;}
...
//		cout << Forme_kppv13::trouver_urgence_max()<<endl;
		Forme_kppv13::the_goban[p]=1;
		n++;
		Fichier_pattern::determiner_et_enlever_pierres_mortes(p,Forme_kppv13::the_goban,1);
	}
