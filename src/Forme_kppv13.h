#ifndef FORME_KPPV13_H
#define FORME_KPPV13_H
...
#define GOBAN_SIZE13_2 169
#define GOBAN_SIZE13 13
...
public:
  ...
  static void init_urgences(Arbre13 *,double,double,double,double,double,double);
  static T_URG de_proba_a_urgence_noir(double,double,double);
  static T_URG de_proba_a_urgence_blanc(double,double,double);
  
	static void construire_liens(Arbre13 *,Arbre13 *,int,int);
	static void lier_pattern(Arbre13 *, Arbre13 *,int,int,double);
	static void verification(void);
	static void effacer_listes(Arbre13 *);
	
	// patterns 3x3 int�r�
	static void tests(void);
	static Forme_kppv13 * generer_le_pattern_3x3(long long int);
	static void generer_patterns_3x3(Arbre13 *,int,long long int);
	static Arbre13 * grimper(Arbre13 *,int);
	...
	static double urgences_noir[GOBAN_SIZE13_2+1];
	static double urgences_blanc[GOBAN_SIZE13_2+1];

	static int the_goban [GOBAN_SIZE13_2+1];	
	static int goban_recure [GOBAN_SIZE13_2+1];	

//	static int tailles_urgences[GOBAN_SIZE13_2];
	static int table_pos_idx_vers_idx[TAILLE_TABLE_POS_IDX_VERS_IDX];
	static Arbre13 * racine_arbre;
	static Arbre13 * racine_arbre_nv;
	static Arbre13 * racine_arbre_nv_bord;
	static Arbre13 * racine_arbre_blanc;
	static Arbre13 * racine_arbre_noir;
	static Liste * liste_des_patterns_blanc;
	static Liste * liste_des_patterns_noir;
	static ifstream * ifile;
	static ofstream * ofile;
	static Arbre13 * arbre_retour;
...
//////////////////////////////////////// Variables non statiques /////////////////////////////////
	...
	//Forme_kppv13 * symetrie_suivante;
// Probas :
	double bonte_noir;
	double bonte_blanc;
	double n_match_noir;
...
// Constructeur
	Forme_kppv13(unsigned char,unsigned short int,unsigned char, long long int);
	~Forme_kppv13();
// Initialisation
	static void trouver_cercles(int);
	static void creer_les_listes(void);
	void mettre_pattern_dans_arbre(Arbre13 *);
//	static void marquer_les_bords(int[GOBAN_SIZE13_2]);
...
//	static void match_77(int);
	...
	static void match_intersection_recurrent(int,Arbre13 *,int );
	static void match_intersection(int &,Arbre13 *,int);


	static int trouver_urgence_max(double *);
// Génération de patterns
	static void generer_un_pattern(int[GOBAN_SIZE13_2],int,unsigned char,int);

// Construction de l'arbre
	static void detruire_feuilles(Arbre13 *);
	static void detruire_arbre(Arbre13 *);
...
// Procédures de lecture écriture écran
	...
	static void ecrire_damier(int[GOBAN_SIZE13_2]);
	static void afficher_arbre(Arbre13 *,int,double,double,unsigned char);
	static void ecrire_arbre(Arbre13 *,int,double,double,double,int);
...
// Procédures de lecture écriture fichiers
	...
	static Forme_kppv13 * lire_pattern(int);
